...
unsigned long int getRegister(jmp_buf *context, int reg) {
    // get registers
    switch(reg) {
        // all registers that need to be demangled
        case JB_PC:
        case JB_RSP:
        case JB_RBP:
        case JB_RBX:
            return ptr_demangle((*context)->__jmpbuf[reg]);

        // all registers that do not need to be demangled
        case JB_R12:
        case JB_R13:
        case JB_R14:
        case JB_R15:
            return (*context)->__jmpbuf[reg];
    }
    return -1;
}
...
void schedule() {
    // save current thread
    printf("Switching from thread %ld\n", gCurrent);
...
int pthread_create(pthread_t *thread, const pthread_attr_t *attr, void *(*start_routine) (void *), void *arg) {
    ...
    *thread = id;
    ...
    void *stackTop = newThread->stack + STACK_SIZE;
    ...
    memcpy(stackPtr, (void *)(&pthread_exit), sizeof(void *));
    ...
    setRegister(&(newThread->context), JB_R12, (unsigned long int)arg);
    setRegister(&(newThread->context), JB_R13, (unsigned long int)start_routine);
